...
databaseDescribe("AUT-101 bounded automatic editorial repair", () => {
  ...
  test("retries a blocked asset twice, never concurrently and then leaves a localized exception", async () => {
    ...
    const firstRepair = await generation.findRequest({ workspaceId, operation: "asset.improve", requestKey: `autopilot:repair:${initial.assetId}:linkedin-editorial-v4:v1` });
    ...
    expect(await reconciler.reconcile()).toBe(1);
    const secondRepair = await generation.findRequest({ workspaceId, operation: "asset.improve", requestKey: `autopilot:repair:${initial.assetId}:linkedin-editorial-v4:v2` });
