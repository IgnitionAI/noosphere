...
(url ? describe : describe.skip)("Agent platform through MCP", () => {
 ...
 const context: McpExecutionContext={workspaceId,userId,clientId,role:"owner",scopes:["mcp:read","mcp:write","mcp:approve"],audience:"https://mcp.example.test/mcp"};
 ...
 beforeAll(async()=>{
  ...
  await db.db.insert(mcpOauthClients).values({clientId,clientName:"Agent test",redirectUris:[],userId,workspaceId,allowedScopes:["mcp:read","mcp:write","mcp:approve"]});
 ...
 test("manual preparation composes drafts and stops before activation or scheduling",async()=>{
  ...
  await db.db.insert(sequenceSteps).values(defaultCampaignSequenceSteps("linkedin").map(step=>({id:crypto.randomUUID(),workspaceId,sequenceId,...step})));
  const offerId=crypto.randomUUID(),offerVersionId=crypto.randomUUID(),planId=crypto.randomUUID(),assessmentId=crypto.randomUUID();
  await db.db.insert(offers).values({id:offerId,workspaceId,name:"Manual offer"});
  await db.db.insert(offerVersions).values({id:offerVersionId,workspaceId,offerId,version:1,name:"Manual offer",category:"service",valueProposition:"Conseil",targetAudience:"PME",publishedAt:new Date()});
  await db.db.insert(prospectingPlans).values({id:planId,workspaceId,icpVersionId,name:"Manual plan",status:"ready"});
  await db.db.insert(channelAssessments).values({id:assessmentId,workspaceId,planId,channel:"linkedin",status:"completed",recommendation:"recommended",strategy:{query:"PME France",sourceKinds:[]}});
  await db.db.insert(campaigns).values({id:campaignId,workspaceId,sequenceId,icpVersionId,offerVersionId,planId,assessmentId,channel:"linkedin",name:"Manual preparation",status:"draft",automationStage:"composing",autopilotPolicy:{activationMode:"manual",enabled:true}});
  await db.db.insert(contacts).values({id:contactId,workspaceId,firstName:"Camille",lastName:"Test"});
  await db.db.insert(prospectDiscoveryRuns).values({id:runId,workspaceId,icpVersionId,filters:{},status:"completed"});
  await db.db.insert(prospectDiscoveryCandidates).values({id:candidateId,workspaceId,runId,fullName:"Camille Test",channels:{linkedin:{value:"https://linkedin.com/in/camille-test",normalizedValue:"linkedin.com/in/camille-test",status:"verified",confidence:"high",source:"fixture"},email:{value:null,normalizedValue:null,status:"unavailable",confidence:"none",source:null},whatsapp:{value:null,normalizedValue:null,status:"unavailable",confidence:"none",source:null}},providerData:{providerId:"fixture-camille"}});
  await db.db.insert(campaignProspects).values({workspaceId,campaignId,candidateId,contactId,eligible:true,state:"imported",score:90});
  ...
  const [job]=await db.db.select().from(jobs).where(eq(jobs.id,jobId));
  expect(job!.status).toBe("completed");
  const repository=new PostgresCampaignRepository(db.db);
  expect(await repository.preflight({workspaceId,campaignId})).toMatchObject({ok:true,blockers:[]});
  await db.db.insert(connectedAccounts).values({workspaceId,provider:"unipile",providerAccountId:"fixture-account",status:"connected",capabilities:{linkedin:true},quotas:{remaining:100},encryptedSecret:"fixture-not-a-secret",lastCheckedAt:new Date()});
  await db.db.insert(workspaceChannelAccounts).values({workspaceId,channel:"linkedin",provider:"unipile",providerAccountId:"fixture-account",displayName:"Fixture account",selectedBy:userId});
  const preparedActivation=await client.callTool({name:"campaign_prepare_activation",arguments:{requestKey:crypto.randomUUID(),campaignId}});
  expect(preparedActivation.structuredContent).not.toHaveProperty("error");
  const proposal=preparedActivation.structuredContent as any;
  const approved=await client.callTool({name:"approval_decide",arguments:{approvalItemId:proposal.approvalItemId,decision:"approve"}});
  expect(approved.isError).not.toBe(true);
  expect(approved.structuredContent).toMatchObject({status:"queued"});
  const proposalId=proposal.proposalId as string;
  const activator=new PostgresGovernedCampaignActivation(db.db);
  const activation={workspaceId,campaignId,proposalId};
  const executor=new PostgresMcpGovernedEffectExecutor(db.db,{campaign:activator});
  const worker=new PostgresMcpGovernedEffectWorker(db.db,new ExternalEffectPolicy(new PostgresExternalEffectFactsReader(db.db)),{queue,now:()=>new Date(),attemptPort:new PostgresMcpExternalEffectAttemptRepository(db.db,executor),executor:async input=>{
  const [approvedAssessment]=await db.db.select().from(channelAssessments).where(eq(channelAssessments.id,assessmentId));
  await db.db.update(channelAssessments).set({strategy:{query:"Changed after approval",sourceKinds:[]}}).where(eq(channelAssessments.id,assessmentId));
  await expect(activator.activate(activation)).rejects.toThrow("MCP_EFFECT_SOURCE_STALE");
  expect(await activator.findReceipt(activation)).toBeNull();
  await db.db.update(channelAssessments).set({strategy:approvedAssessment!.strategy}).where(eq(channelAssessments.id,assessmentId));
    return executor.execute(input);
  }});
  const execution=(await queue.lease({workerId:"governed-activation",types:["mcp.external-effect.execute"],limit:100,leaseMs:60000,now:new Date()})).find(row=>row.id===(approved.structuredContent as any).jobId)!;
  await worker.process({...execution,status:"running"});
  const status=await client.callTool({name:"approval_get",arguments:{proposalId}});
  expect(status.structuredContent).toMatchObject({status:"delivered"});
  const receipt=await activator.findReceipt(activation);
  expect(receipt).not.toBeNull();
  if (!receipt) throw new Error("ACTIVATION_RECEIPT_REQUIRED");
  expect(await activator.activate(activation)).toEqual(receipt);
  const activationJobs=(await db.db.select().from(jobs).where(and(eq(jobs.workspaceId,workspaceId),eq(jobs.type,"campaign.messages.compose"),eq(jobs.status,"pending")))).filter(row=>(row.payload as {campaignId?:string}).campaignId===campaignId);
  expect(activationJobs).toHaveLength(1);
  const activationJob=(await queue.lease({workerId:"activation-worker",types:["campaign.messages.compose"],limit:100,leaseMs:60000,now:new Date()})).find(row=>row.id===activationJobs[0]!.id)!;
  await new CampaignCompositionJobProcessor(db.db,queue,{generate:async()=>{throw new Error("PREPARED_MESSAGES_MUST_BE_REUSED");}},{resolveHealthyAccount:async()=>({provider:"unipile",accountId:"fixture-account"})},{now:()=>new Date()}).process(activationJob);
  const scheduled=(await client.callTool({name:"campaign_list",arguments:{}})).structuredContent as any;
  expect(scheduled.data.find((row:any)=>row.id===campaignId)).toMatchObject({status:"active",automationStage:"scheduled"});
  expect((await db.db.select().from(outreachActions).where(eq(outreachActions.campaignId,campaignId))).length).toBeGreaterThan(0);
  await repository.transition({workspaceId,campaignId,userId,transition:"pause",at:new Date()});
  expect(await activator.activate(activation)).toEqual(receipt);
  expect(await activator.findReceipt(activation)).toEqual(receipt);
  const final=(await client.callTool({name:"campaign_list",arguments:{}})).structuredContent as any;
  expect(final.data.find((row:any)=>row.id===campaignId).status).toBe("paused");
