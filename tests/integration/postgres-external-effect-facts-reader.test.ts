...
databaseDescribe("Postgres external-effect facts reader", () => {
  ...
  test("projects the latest content publication and blocks stale readiness or strategy snapshots", async () => {
    ...
    await expect(database.db.transaction(async (tx) => {
      ...
      await tx.insert(editorialStrategies).values({ id: ids.strategy, workspaceId, name: "Content strategy", offerId: ids.offer, offerVersionId: ids.offerVersion, icpId: ids.icp, icpVersionId: ids.icpVersion, status: "active", currentVersion: 1, draft: {}, provider: "fixture", model: "fixture", promptVersion: "v1", createdAt: now, updatedAt: now });
