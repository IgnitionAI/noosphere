...
(url ? describe : describe.skip)("AI mission manual resume", () => {
  ...
  test("quota pause survives worker restart and concurrent manual resume preserves completed checkpoints", async () => {
    ...
    try {
      ...
      try {
        ...
        await Promise.all([1, 2].map(() => new ResumeProductResearchRun(restartedResearch, ids, clock).execute({ workspaceId, runId: run.snapshot.id, correlationId: "manual-resume" })));
