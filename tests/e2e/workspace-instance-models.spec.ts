...
test("workspaces inherit or customize authorized models and explain withdrawn defaults", async ({ page }) => {
  ...
  try {
    ...
    async function defaultTo(route: typeof a) {
      expect((await page.request.post(`${api}/api/v1/instance/ai/default`, { data: { connectionId: route.connectionId, model: route.model } })).status()).toBe(200);
    }
    await defaultTo(a);
    ...
    await expect(page.getByText(`Modèle effectif : ${a.model}`, { exact: true })).toBeVisible();
