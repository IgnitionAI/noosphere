  ...
  return (
    <>
      ...
      {editable ? <section className="panel mt-5"><div className="panel-header"><h2 className="font-semibold">Décision humaine</h2></div><div className="panel-body grid gap-4 md:grid-cols-2">
        ...
        <MutationForm action={reject} confirmation="Confirmer le rejet ? La justification sera conservée dans l’audit." successMessage="Item rejeté."><label className="text-xs font-semibold text-ink">Justification obligatoire<textarea className="control mt-1 min-h-24 w-full" name="justification" placeholder="Expliquez ce rejet…" required /></label><button className="button mt-2 w-full" type="submit">Rejeter</button></MutationForm>
      </div></section> : null}
...
function ContentPanel({ label, value, edited = false, action }: { label: string; value: unknown; edited?: boolean; action?: (formData: FormData) => Promise<unknown> }) {
  ...
  return <section className="panel min-w-0"><div className="panel-header"><h2 className="font-semibold">{label}</h2></div><div className="panel-body">{edited && action ? <MutationForm action={action} successMessage="Contenu édité enregistré."><textarea className="control min-h-72 w-full font-mono text-xs" defaultValue={text} name="contentEdited" aria-label={label} /><button className="button mt-3" type="submit">Enregistrer la modification</button><p className="mt-2 text-[11px] text-muted">JSON valide accepté pour conserver la structure ; sinon le texte est envoyé tel quel.</p></MutationForm> : <pre className="max-h-[28rem] overflow-auto whitespace-pre-wrap break-words rounded-lg bg-slate-50 p-4 text-xs leading-5 text-ink">{text}</pre>}</div></section>;
}
