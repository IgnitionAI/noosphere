...
export default async function WorkspaceSettingsPage({ params, searchParams }: { params: Promise<{ workspaceSlug: string }>; searchParams: Promise<{ notice?: string; error?: string; exportId?: string; action?: string; actorUserId?: string; from?: string; to?: string }> }) {
  ...
  const instanceSetup = await getInstanceSetup();
  ...
  return <div className="mx-auto max-w-6xl">
    {instanceSetup.isAdministrator ? <Link className="button mb-4" href="/settings/instance/ai">IA de l’instance</Link> : null}
...
function SettingLink({ href, icon, label, detail }: { href: string; icon: React.ReactNode; label: string; detail: string }) { return <Link className="panel flex min-h-20 items-center gap-3 p-4 transition-colors hover:border-slate-300 hover:bg-slate-50" href={href}><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-navy text-signal">{icon}</span><span className="min-w-0"><strong className="block text-sm">{label}</strong><span className="mt-0.5 block text-[11px] text-muted">{detail}</span></span></Link>; }
