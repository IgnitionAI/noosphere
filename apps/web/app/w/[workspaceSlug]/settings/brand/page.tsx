...
export default async function WorkspaceBrandPage({ params }: { params: Promise<{ workspaceSlug: string }> }) {
  ...
  return <div className="mx-auto max-w-6xl">
    <header className="border-b border-line pb-6">
      <Link className="inline-flex items-center gap-1 text-xs font-semibold text-muted hover:text-ink" href={`/w/${workspaceSlug}/settings`}><ArrowLeft size={13} /> Configuration</Link>
      ...
      <p className="mt-2 max-w-3xl text-sm leading-6 text-muted">Noosphere réutilise automatiquement cette identité dans vos posts, images, messages Outbound et réponses du Setter.</p>
    </header>
