    ...
    return (
      <>
        ...
        <section aria-label="Indicateurs de la lens" className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {activity.counters.map((counter) => <article className="panel p-4" key={counter.key}><p className="text-xs text-muted">{counter.label}</p><strong className="metric-value mt-2 block text-ink">{counter.value}</strong></article>)}
        </section>
...
function statusClass(state: ActivityWorkspacePage["state"]): string {
  ...
  if (state === "not_configured") return "flex flex-col gap-3 rounded-xl border border-line bg-white p-4 text-ink sm:flex-row sm:items-center sm:justify-between";
