    ...
    <div className="grid gap-2 sm:grid-cols-3">
      {formats.map((item) => {
        ...
        return <article className={`rounded-xl border p-4 transition-colors ${enabled ? "border-navy bg-navy/[0.035]" : "border-line bg-surface-subtle opacity-70"}`} key={item.format}>
          <div className="flex items-start justify-between gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-white text-navy shadow-sm"><Icon size={17} /></span><button aria-pressed={enabled} className={`rounded-full px-3 py-1 text-xs font-bold ${enabled ? "bg-navy text-white" : "bg-line text-muted"}`} disabled={pending} onClick={() => toggle(item.format)} type="button">{enabled ? "Actif" : "Inactif"}</button></div>
          <h3 className="mt-3 font-semibold text-navy">{item.label}</h3><p className="mt-1 min-h-10 text-xs leading-5 text-muted">{item.detail}</p>
