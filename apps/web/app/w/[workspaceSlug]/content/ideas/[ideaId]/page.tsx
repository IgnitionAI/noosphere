...
function MediaPreview({ plan, media }: { plan: ContentMediaPlan; media: NonNullable<Awaited<ReturnType<typeof getContentIdeaDetail>>["asset"]>["latest"] extends infer L ? L extends { media: infer M } ? M : never : never }) {
  ...
  return <div className="border-t border-line bg-surface-subtle p-5"><div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-semibold text-navy">Aperçu {formatLabel(plan.format)}</h3><p className="mt-1 text-xs text-muted">{media ? `${media.filename} · ${formatBytes(media.sizeBytes)}` : "Rendu après validation éditoriale"}</p></div>{media ? <span className="badge badge-success">Fichier prêt</span> : <span className="badge">Prévisualisation</span>}</div><div className={`mt-4 grid gap-3 ${cards.length > 1 ? "sm:grid-cols-2 lg:grid-cols-3" : "max-w-md"}`}>{cards.map((card, index) => <article className="flex aspect-[4/5] min-h-64 flex-col justify-between rounded-2xl border border-navy/10 bg-[#F7F8F4] p-5 shadow-sm" key={`${card.title}:${index}`}><div><div className="flex items-center justify-between gap-2"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-navy/60">Noosphere {cards.length > 1 ? `· ${index + 1}/${cards.length}` : ""}</p>{"layout" in card && card.layout && card.layout !== "auto" ? <span className="rounded-full bg-lime/25 px-2 py-1 text-[9px] font-bold uppercase tracking-wide text-navy">{carouselLayoutLabel(card.layout)}</span> : null}</div>{"kicker" in card && card.kicker ? <p className="mt-5 text-[10px] font-bold uppercase tracking-[0.18em] text-signal">{card.kicker}</p> : null}<h4 className="mt-4 text-xl font-bold leading-tight text-navy">{card.title}</h4><p className="mt-3 text-sm leading-6 text-navy/70">{card.body}</p>{"callout" in card && card.callout ? <div className="mt-4 rounded-xl border-l-4 border-signal bg-signal/5 p-3 text-xs font-semibold leading-5 text-navy">{card.callout}</div> : null}{"items" in card && card.items?.length ? <ul className="mt-4 space-y-2">{card.items.map((item) => <li className="rounded-lg bg-navy/[0.04] p-2.5 text-[11px] leading-4 text-navy" key={`${item.label}:${item.text}`}><strong>{item.label}</strong><span className="ml-1 text-navy/65">{item.text}</span></li>)}</ul> : null}</div><div className="flex items-end justify-between border-t border-navy/10 pt-4 text-[10px] font-semibold text-navy"><span>Créer et capter la demande</span>{"durationSeconds" in card && card.durationSeconds !== undefined ? <span>{card.durationSeconds}s</span> : null}</div></article>)}</div></div>;
}
