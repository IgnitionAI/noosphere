...
import { AuditObjections } from "./audit-objections";
  ...
  return <>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(300px,.6fr)]">
      <main className="space-y-5">
        {latest ? <section className="panel overflow-hidden"><div className="panel-header flex items-center justify-between gap-3"><div><h2 className="font-semibold">Version {latest.version}</h2><p className="mt-1 text-xs text-muted">Snapshot éditorial et média immuables</p></div><span className={latest.readiness.ready ? "badge badge-success" : "badge badge-warning"}>{latest.readiness.ready ? "Prête" : latest.readiness.blockers.includes("editorial_policy_outdated") ? "À réévaluer" : "Bloquée"}</span></div><article className="whitespace-pre-wrap p-6 text-[15px] leading-7 text-ink">{latest.body}</article>{latest.draft.mediaPlan && latest.draft.mediaPlan.format !== "linkedin_text" ? <MediaPreview media={latest.media} plan={latest.draft.mediaPlan} /> : null}</section> : <section className="panel py-16 text-center"><PenLine className="mx-auto text-muted" size={30} /><h2 className="mt-4 font-semibold">Aucun contenu rédigé</h2><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-muted">Le pipeline transforme cette idée en brief, rédige, vérifie chaque preuve puis lance une critique indépendante.</p></section>}

        {latest ? <AuditObjections audit={latest.audit} /> : null}

        {latest ? <details className="panel group"><summary className="panel-header cursor-pointer list-none"><div><h2 className="font-semibold">Évaluation du contenu</h2><p className="mt-1 text-xs text-muted">Voir l’audit éditorial et les éventuels blocages.</p></div><span className="badge">Détails</span></summary><div className="panel-body space-y-4 border-t border-line"><p className="text-sm leading-6 text-muted">{latest.critique.summary}</p>{latest.critique.qualityAssessment ? <dl className="space-y-3">{Object.entries(latest.critique.qualityAssessment).map(([criterion, review]) => <div className="rounded-lg border border-line p-3" key={criterion}><dt className="text-sm font-semibold text-ink">{qualityCriterionLabel(criterion)} · {review.verdict === "pass" ? "Validé" : "À retravailler"}</dt><dd className="mt-1 text-sm leading-6 text-muted">{review.reason}</dd>{review.excerpts.map((excerpt, index) => <dd className="mt-2 border-l-2 border-line pl-3 text-xs leading-5 text-muted" key={index}>« {excerpt} »</dd>)}</div>)}</dl> : <p className="text-sm text-muted">Cette version doit être réévaluée selon les critères éditoriaux actuels avant toute nouvelle programmation.</p>}{latest.readiness.blockers.length ? <div className="rounded-xl border border-amber-200 bg-amber-50 p-4"><strong className="text-sm text-amber-950">Blocages</strong><div className="mt-2 flex flex-wrap gap-2">{latest.readiness.blockers.map((blocker) => <span className="badge badge-warning" key={blocker}>{blockerLabel(blocker)}</span>)}</div></div> : <div className="flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-950"><CheckCircle2 className="mt-0.5" size={17} /><div><strong className="text-sm">Qualité éditoriale validée</strong><p className="mt-1 text-xs">Hook spécifique, CTA aligné et aucune répétition bloquante détectée.</p></div></div>}{latest.critique.issues.length ? <ul className="space-y-2">{latest.critique.issues.map((issue, index) => <li className="rounded-lg bg-slate-50 p-3 text-xs leading-5 text-muted" key={`${issue.code}:${index}`}><strong className="text-ink">{issue.code}</strong> · {issue.message}</li>)}</ul> : null}</div></details> : null}
      ...
      <aside className="space-y-5">
        ...
        {latest ? <details className="panel"><summary className="panel-header cursor-pointer list-none"><div><h2 className="font-semibold">Vérification des faits</h2><p className="mt-1 text-xs text-muted">Traçabilité du texte</p></div><span className="badge">Voir</span></summary><div className="panel-body space-y-3 border-t border-line">{latest.audit.reviewedClaims.length ? latest.audit.reviewedClaims.map((claim, index) => <div className="rounded-lg bg-slate-50 p-3" key={`${claim.statement}:${index}`}><div className="flex items-center gap-2"><FileCheck2 className={claim.verdict === "supported" ? "text-success" : "text-danger"} size={14} /><strong className="text-xs text-ink">{claim.verdict === "supported" ? "Prouvé" : "Non prouvé"}</strong></div><p className="mt-2 text-xs leading-5 text-muted">{claim.statement}</p><p className="mt-2 text-xs leading-5 text-muted">{claim.reason}</p><p className="mt-2 text-[11px] text-muted">{claim.sourceKeys.join(" · ") || "Aucune preuve"}</p></div>) : <p className="text-xs text-muted">Aucun fait déclaré dans cet audit. Consultez l’évaluation et les éventuelles objections avant de conclure.</p>}</div></details> : null}
      </aside>
    </div>
  </>;
}
...
function qualityCriterionLabel(criterion: string) { return ({ audienceRelevance: "Pertinence pour la cible", readerValue: "Utilité pour le lecteur", coherence: "Clarté du raisonnement", sourceAttribution: "Sources identifiables", ctaTruthfulness: "Appel à l’action vérifiable", brandVoice: "Voix de marque", distinctness: "Différence avec les posts précédents" } as Record<string, string>)[criterion] ?? criterion; }
function blockerLabel(blocker: string) { if (blocker.startsWith("editorial_") && !["editorial_blocker", "editorial_assessment_missing", "editorial_assessment_invalid", "editorial_policy_outdated"].includes(blocker)) return qualityCriterionLabel(blocker.slice("editorial_".length)); return ({ editorial_policy_outdated: "Version à réévaluer", editorial_assessment_missing: "Évaluation éditoriale à refaire", editorial_assessment_invalid: "Évaluation éditoriale incomplète", unsupported_claim: "Fait non prouvé", unresolved_audit_claim: "Objection factuelle à corriger", unresolved_audit_scenario: "Scénario contesté à corriger", unresolved_audit_topic: "Objection sur un sujet à résoudre", topic_audit_invalid: "Passage interdit à vérifier", misleading_scenario: "Scénario trompeur", unaudited_claim: "Fait non audité", ungrounded_statement: "Phrase non sourcée", forbidden_topic: "Sujet interdit", generic_language: "Langage générique", repetition: "Répétition", cta_misaligned: "CTA hors offre", editorial_blocker: "Critique bloquante" } as Record<string, string>)[blocker] ?? blocker; }
