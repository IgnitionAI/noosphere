import { AlertTriangle, ArrowLeft, CheckCircle2, Clock3, ExternalLink, Lightbulb, Radar, ShieldCheck } from "lucide-react";
...
export default async function ContentIdeasPage({ params, searchParams }: { params: Promise<{ workspaceSlug: string }>; searchParams: Promise<Query> }) {
  ...
  return <>
    ...
    {ideas.data.length ? <section className="mt-4 grid gap-4 xl:grid-cols-2">{ideas.data.map((idea) => <article className="panel p-5" key={idea.id}><div className="flex items-start justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap gap-2"><span className="badge badge-signal">{idea.pillar}</span><span className="badge">{idea.audience}</span></div><h2 className="mt-3 text-lg font-semibold leading-7 text-navy">{idea.angle}</h2></div><strong className="shrink-0 text-xl text-navy">{idea.priority}</strong></div><p className="mt-3 text-sm leading-6 text-muted">{idea.rationale}</p><div className="mt-4 flex items-center justify-between border-t border-line pt-4 text-xs text-muted"><span className="inline-flex items-center gap-1"><Clock3 size={13} /> Frais jusqu’au {formatDate(idea.freshnessUntil)}</span><span className="inline-flex items-center gap-1 font-semibold text-success"><ShieldCheck size={13} /> {idea.sources.length} preuve{idea.sources.length === 1 ? "" : "s"}</span></div><details className="mt-3 rounded-lg bg-slate-50 p-3"><summary className="cursor-pointer text-xs font-semibold text-navy">Voir les preuves résolubles</summary><ul className="mt-3 space-y-3">{idea.sources.map((source) => <li className="text-xs leading-5 text-muted" key={`${source.contentHash}:${source.sourceRef}`}><strong className="block text-navy">{source.title}</strong><span>{source.excerpt}</span>{source.canonicalUrl ? <a className="ml-2 inline-flex items-center gap-1 font-semibold text-signal" href={source.canonicalUrl} rel="noreferrer" target="_blank">Source <ExternalLink size={11} /></a> : <span className="ml-2 badge">{sourceType(source.type)}</span>}</li>)}</ul></details></article>)}</section> : <section className="panel mt-4 py-16 text-center"><Lightbulb className="mx-auto text-muted" size={30} /><h2 className="mt-4 font-semibold">Aucune idée dans ce filtre</h2><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-muted">Activez d’abord la stratégie, puis lancez le radar. Les résultats apparaissent sans quitter la page.</p></section>}
