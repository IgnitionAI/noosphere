    ...
    <>
      <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="page-title mt-3">Rendez-vous</h1>
        ...
        <section className="mb-5 rounded-xl border border-lime-300 bg-lime-50/60 p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div><p className="text-[11px] font-semibold uppercase tracking-wide text-muted">Prochain appel</p><h2 className="mt-1 font-semibold text-navy">{displayName(next)}</h2><p className="mt-1 text-sm text-muted">{formatDate(next.startAt)} · {duration(next)}</p></div>
            </div>
          ...
          <div className="divide-y divide-line">
            {visible.map((booking) => (
              <article className="grid gap-4 p-5 md:grid-cols-[160px_minmax(0,1fr)_170px_auto] md:items-center" key={booking.id}>
                ...
                <div><span className={statusBadge(booking.status)}>{statusLabel(booking.status)}</span>{booking.rescheduleCount ? <p className="mt-1 text-[11px] text-muted">Replanifié {booking.rescheduleCount} fois</p> : null}</div>
...
function displayName(booking: CalendarBooking): string { return booking.attendeeName?.trim() || booking.attendeeEmail || booking.attendeePhone || "Prospect"; }
...
function statusLabel(status: string): string { return ({ scheduled: "Confirmé", rescheduled: "Replanifié", completed: "Terminé", cancelled: "Annulé", no_show: "Absent" } as Record<string, string>)[status] ?? status; }
function statusBadge(status: string): string { if (["scheduled", "rescheduled"].includes(status)) return "badge badge-success"; if (["cancelled", "no_show"].includes(status)) return "badge badge-warning"; return "badge"; }
