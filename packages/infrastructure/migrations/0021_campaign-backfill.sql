-- Earlier V2 reports also kept only rank 1 as an operational version. Materialize
-- every ranked proposal from V2+ without changing its research content.
...
-- Create one isolated draft sequence and discovery run for every ICP version that
-- predates automatic campaign creation. Deterministic IDs make the backfill safe.
