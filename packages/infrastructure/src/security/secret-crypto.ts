...
function key(masterKey?: string): Buffer {
  const value = masterKey ?? process.env.APP_ENCRYPTION_KEY ?? process.env.BETTER_AUTH_SECRET;
...
export function decryptSecret(envelope: string, masterKey?: string): string {
  ...
  const decipher = createDecipheriv("aes-256-gcm", key(masterKey), Buffer.from(ivEncoded, "base64url"));
