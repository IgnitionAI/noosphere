  ...
  async search(input: { readonly query: string; readonly limit: number; readonly model: string; readonly reasoningEffort: AiReasoningEffort; readonly deadlineAt: Date; readonly signal?: AbortSignal }): Promise<{
    ...
    try {
      ...
      const result = await this.#runner.run({
        ...
        stdin: ["Find substantive primary sources for an editorial research brief using native web_search.",
          "The supplied query describes an audience and editorial intent; it is not necessarily a literal search-engine query.",
          "Identify the concrete activity, question or decision behind it, then formulate concise searches in the terminology used by practitioners.",
          "Search complementary phrasings or another language when useful; preserve the audience's actual problem and do not introduce an unrelated topic.",
          "Prefer pages that explain a practice, observed example, tradeoff or original finding over contact pages, login portals, generic service pages or job descriptions.",
          "Return fewer results if the evidence is weak.",
