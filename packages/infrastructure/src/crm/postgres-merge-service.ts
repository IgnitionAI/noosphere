...
export class PostgresMergeService {
  ...
  async undo(input: { workspaceId: string; contactId: string; undoneBy: string }) {
    return this.db.transaction(async (tx) => {
      ...
      for (const identity of snapshot.identities) await tx.update(contactIdentities).set({ contactId: String(identity.contactId), type: identity.type as "email" | "linkedin" | "phone" | "whatsapp", value: String(identity.value), normalizedValue: String(identity.normalizedValue), source: identity.source as "manual" | "csv" | "icp_research" | "discovery" | "provider" }).where(and(eq(contactIdentities.workspaceId, input.workspaceId), eq(contactIdentities.id, String(identity.id))));
